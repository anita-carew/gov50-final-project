...
ggplot(final_df, aes(x = year, y = GDP)) +
  geom_line() 

  ggplot(final_df, aes(x = year, y = CO2)) +
  geom_line() 

#returns multiple lines
ggplot(final_df, aes(x = year, group = country_name)) +
...
#tried making each year into a mean but it returns a blank graph
agg_data <- final_df |>
  group_by(year) |>
  summarise(
    CO2 = mean(CO2), 
...
December 8th - Things still missing for my final project

One line chart graph that has one line showing CO2 over time and one line showing GDP over time. This will show the relationship of the variables. Use final_df for this

One regression table. Use final_df for this.

Everything else just write up the lab report.
